	...
	private function get_harga_per_tindakan($parameter) {
		...
		foreach ($kelas['response_data'] as $key => $value) {
			...
			if(count($data['response_data']) > 0) {
				...
				$Tindakan = self::get_tindakan_detail($data['response_data'][0]['tindakan']);
				$data['response_data'][0]['tindakan_detail'] = $Tindakan['response_data'][0];

				//Kelas Detail
				$KelasDetail = self::get_kelas_tindakan_detail($data['response_data'][0]['kelas']);
				$data['response_data'][0]['kelas'] = $KelasDetail['response_data'][0];
